...
class ToySpuriousCorrelationsConceptDataset(ToyGraphClassificationDataset):
    def __init__(self, root, n_graphs=1000, n_node_features=1):
        super(ToySpuriousCorrelationsConceptDataset, self).__init__(root=root, transform=None, pre_transform=None, pre_filter=None)
        self.name = 'ToySpuriousDataset'
    ...
    def _generate_graph(self, graph_id):
        ...
        if class_label == 1:
            ...
            add_spurious_signal = bool(random.uniform(0, 1.0) > 0.2)
    ...
    def _attach_square_to_graph(self, graph):
        ...
        return {"features": node_features, "edges": edge_list, "colors": graph['colors'] + ['red' for _ in range(n_new_nodes)]}
